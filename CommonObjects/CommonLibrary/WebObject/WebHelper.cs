...
namespace CommonLibrary.WebObject
{
    public class WebHelper
    {
        ...
        {
            string path = "";
            if (String.Equals(System.Environment.CurrentDirectory.TrimEnd('\\'), AppDomain.CurrentDomain.BaseDirectory.TrimEnd('\\'), StringComparison.InvariantCultureIgnoreCase))
            {
        ...
        public static bool IsIE(System.Web.HttpRequest request)
        {
            return request.Browser.Browser.ToUpper().IndexOf("IE") >= 0;
        }
        ...
        public static bool IsIE9(System.Web.UI.Page page)
        {
            return IsIE(page) && page.Request.Browser.Version.IndexOf("9.0") >= 0;
        }

        public static bool IsWebkit(System.Web.UI.Page page)
        {
            return page.Request.ServerVariables["HTTP_USER_AGENT"].ToLower().IndexOf("firefox") > 0;
        }
            ...
            foreach (string k in GetRequestKeys(method))
            {
                if (k.StartsWith(prefix))
                {
                    ...
                    if (string.IsNullOrEmpty(column))
                        continue;
                    //DataMapping.ObjectHelper.SetValue(o, column, System.Web.HttpUtility.UrlDecode(System.Web.HttpContext.Current.Request[k]), CommonLibrary.Utility.DateHelper.GetDateString(CommonLibrary.Utility.DateHelper.DefaultDateFormat));
        ...
        public static void SetValues<T>(T o, string prefix)
            where T : class, new()
        {
            SetValues(o, prefix, CommonLibrary.Utility.DateHelper.GetDateString(CommonLibrary.Utility.DateHelper.DefaultDateFormat), RequestPostMethod.Either);
        }

        public static List<T> GetListFromRequest<T>(string prefix)
            where T : class, new()
        {
            return GetListFromRequest<T>(prefix, CommonLibrary.Utility.DateHelper.DefaultDateFormat, RequestPostMethod.Either);
        }
        ...
        public static List<T> GetListFromRequest<T>(string prefix, Utility.DateHelper.DateFormat dateFormat, RequestPostMethod method)
            where T : class, new()
        {
            return GetListFromRequest<T>(prefix, CommonLibrary.Utility.DateHelper.DefaultDateFormat, method, null);
        }
            ...
            if (maxId > 0)
            {
                for (int i = 1; i <= maxId; i++)
                {
                    ...
                    if (isValid)
                    {
                        ...
                        {
                            T obj = new T();
                            WebHelper.SetValues<T>(obj, string.Concat(prefix, i), Utility.DateHelper.GetDateString(dateFormat), method);
