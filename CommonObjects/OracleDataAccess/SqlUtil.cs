...
namespace OracleDataAccess
{
    public class SqlUtil
    {
        ...
        {
            IDataReader read = OracleDataAccess.SqlUtil.ExecuteReader(conn, @"select sysdate from dual");
            using (read)
            {
        ...
        public static T GetExecuteScalar<T>(IDbConnection conn, string cmdText)
        {
        ...
        public static T GetExecuteScalarByParsCollection<T>(IDbConnection conn, string tableName, ParameterCollection pc)
        {
        ...
        public static T GetRecordsCountByParsCollection<T>(IDbConnection conn, string tableName, ParameterCollection pc)
        {
        ...
        public static void FillCommandByParsCollection(IDbCommand cmd, ParameterCollection pc)
        {
            foreach (Parameter p in pc)
            {
                if (p.TokType != TokenTypes.IsNotNull && p.TokType != TokenTypes.IsNull && p.Column != null && p.TokType != TokenTypes.In && p.TokType != TokenTypes.NotIn)
                    cmd.Parameters.Add(new OracleParameter(string.Format(SqlScriptHandler.ValueTmepString, p.ColumnPar), SqlScriptHandler.ProcessValue(p.TokType, p.Value)));
            }
        }
        ...
        public static IDataReader ExecuteProcedureReader(IDbConnection conn, string storedProcName, OracleParameter parameters)
        {
            return ExecuteProcedureReader(conn, storedProcName, new OracleParameter[] { parameters });
        }
        ...
        {
            return ExecuteDataSet((OracleCommand)GetCommandByScript(conn, cmdText));
        }
            ...
            where C : ICollection<T>, new()
        {
            ...
            return OracleDataAccess.SqlUtil.GetObjectList<T, C>(cmd);
        }
        ...
        {
            IDbCommand cmd = OracleDataAccess.SqlUtil.GetCommandByScript(conn, SqlScriptHandler.Search.GetSelectString(tableName, pc, asc, count, orderBy));
            FillCommandByParsCollection(cmd, pc);
            return cmd;
        }
        ...
        public static int GetIdentityId(IDbConnection conn, string sequenceName)
        {
            IDataReader idr = ExecuteReader(conn, string.Format(SqlScriptHandler.Search.SelectIdentityId, sequenceName));
            using (idr)
            {
                int ret = 0;
                if (idr.Read())
        ...
        public static int GetIdentityId(IDbConnection conn, IDbTransaction tran, string sequenceName)
        {
            IDataReader idr = ExecuteReader(conn, tran, string.Format(SqlScriptHandler.Search.SelectIdentityId, sequenceName));
            using (idr)
            {
                int ret = 0;
                if (idr.Read())
        ...
        {
            string sequenceIdName;
            IDbCommand cmd = GetInsertIDbCommand(conn, null, tableName, t, out sequenceIdName);
            Log.LogCommand(cmd);
            int rowAff = cmd.ExecuteNonQuery();
            if (rowAff > 0)
            {
                if (string.IsNullOrEmpty(sequenceIdName))
                    return rowAff;
                else
                    return GetIdentityId(cmd.Connection, sequenceIdName);
            }
            return 0;
        }
        ...
        {
            string sequenceIdName;
            int rowAff = GetInsertIDbCommand(conn, tran, tableName, t, out sequenceIdName).ExecuteNonQuery();
            if (rowAff > 0)
            {
                if (string.IsNullOrEmpty(sequenceIdName))
                    return rowAff;
                else
                    return GetIdentityId(conn, tran, sequenceIdName);
            }
            return 0;
        }

        public static IDbCommand GetInsertIDbCommand<T>(IDbConnection conn, IDbTransaction tran, string tableName, T t, out string sequenceIdName) where T : class, new()
        {
            sequenceIdName = null;
            ...
            {
                if (mappingInfos[i].SpecialAttributes != null && mappingInfos[i].SpecialAttributes.ToString().ToUpper().IndexOf(SqlScriptHandler.UnInsert) > 0) continue;
                bool isIdentity = false;
                object value = null;
                if (mappingInfos[i].FieldGenInfo != null && mappingInfos[i].FieldGenInfo.FieldGenerateMethod == FieldGenerateMethod.Native)
                {
                    isIdentity = true;
                    sequenceIdName = mappingInfos[i].FieldGenInfo.FieldGenerateMethodExtraInfo != null ? mappingInfos[i].FieldGenInfo.FieldGenerateMethodExtraInfo.ToString() : "";
                }
                else
                {
                    value = mappingInfos[i].PropertyInfo.GetValue(t, null);
                }
                ...
                else if (isIdentity)
                {
                    if (sbValues.Length > 0)
                        sbValues.Append(",");
                    sbValues.Append(string.Format(SqlScriptHandler.Search.NextIdentityId, mappingInfos[i].FieldGenInfo.FieldGenerateMethodExtraInfo));
                }
                else
                {
                    if (sbValues.Length <= 0)
                        sbValues.Append(string.Format(SqlScriptHandler.ValueInsertTempString, mappingInfos[i].DataFieldName));
                    else
                        sbValues.Append(",").Append(string.Format(SqlScriptHandler.ValueInsertTempString, mappingInfos[i].DataFieldName));
                }
                if (!isIdentity)
        ...
        public static IDbCommand GetInsertIDbCommandByUOPars(IDbConnection conn, IDbTransaction tran, string tableName, ParameterCollection insertParameters)
        {
            ...
            foreach (Parameter p in insertParameters)
            {
                ...
                if (sbValues.Length <= 0)
                    sbValues.Append(string.Format(SqlScriptHandler.ValueInsertTempString, p.Column));
                else
                    sbValues.Append(",").Append(string.Format(SqlScriptHandler.ValueInsertTempString, p.Column));
        ...
        public static IDbCommand GetUpdateIDbCommand<T>(IDbConnection conn, IDbTransaction tran, string tableName, T t, ParameterCollection pc, bool isUpdateAll) where T : class, new()
        {
            ...
            for (int i = 0; i < mappingInfos.Count; i++)
            {
                if (mappingInfos[i].SpecialAttributes != null && mappingInfos[i].SpecialAttributes.ToString().ToUpper().IndexOf(SqlScriptHandler.UnUpdate) > 0) continue;
        ...
        public static IDbCommand GetUpdateIDbCommandByUOPars(IDbConnection conn, IDbTransaction tran, string tableName, ParameterCollection pcValues, ParameterCollection pcConditions)
        {
            ...
            foreach (Parameter cp in pcConditions)
            {
                if (cp.TokType != TokenTypes.In && cp.TokType != TokenTypes.NotIn)
                    cmd.Parameters.Add(new OracleParameter(string.Format(SqlScriptHandler.ValueTmepString, cp.Column), SqlScriptHandler.ProcessValue(cp.TokType, cp.Value)));
            }
            if (tran != null)
                cmd.Transaction = tran;
            return cmd;
