...
namespace OracleDataAccess.Data
{
        ...
        where C : ICollection<T>, new()
    {
        ...
        public int Insert()
        {
            ...
            {
                return OracleDataAccess.SqlUtil.ExecuteInsert<T>(conn, ConnInfo.TableName, this as T);
            }
        }

        public int Insert(IDbConnection cnn, IDbTransaction tran)
        {
            return OracleDataAccess.SqlUtil.ExecuteInsert<T>(cnn, tran, ConnInfo.TableName, this as T);
        }
        #endregion
        ...
        public int Update(ParameterCollection pc)
        {
            using (System.Data.IDbConnection conn = ConnInfo.Connection)
            {
                return OracleDataAccess.SqlUtil.ExecuteUpdate<T>(conn, ConnInfo.TableName, this as T, pc, false);
            }
        }

        public int Update(IDbConnection cnn, IDbTransaction tran, ParameterCollection pc)
        {
            return OracleDataAccess.SqlUtil.ExecuteUpdate<T>(cnn, tran, ConnInfo.TableName, this as T, pc, false);
        }
        ...
        public int UpdateAllColumns(ParameterCollection pc)
        {
            using (System.Data.IDbConnection conn = ConnInfo.Connection)
            {
                return OracleDataAccess.SqlUtil.ExecuteUpdate<T>(conn, ConnInfo.TableName, this as T, pc, true);
            }
        }

        public int UpdateAllColumns(IDbConnection cnn, IDbTransaction tran, ParameterCollection pc)
        {
            return OracleDataAccess.SqlUtil.ExecuteUpdate<T>(cnn, tran, ConnInfo.TableName, this as T, pc, true);
        }
