    ...
    public static class ObjectHelper
    {
        ...
        private static List<PropertyMappingInfo> LoadPropertyMappingInfo(Type objType)
        {
            ...
            foreach (PropertyInfo info in objType.GetProperties())
            {
                ...
                if (mapAttr == null && ((info.PropertyType.IsGenericType) && (info.PropertyType.GetGenericTypeDefinition() == typeof(Nullable<>)) || System.Type.GetTypeCode(info.PropertyType) != TypeCode.Object || info.PropertyType == new System.TimeSpan().GetType()))
                {
                ...
                if (mapAttr != null)
                {
                    PropertyMappingInfo mapInfo = new PropertyMappingInfo(mapAttr.DataFieldName, mapAttr.DefaultValue, info, mapAttr.SpecialAttributes, new FieldGenerateInfo(mapAttr.FieldGenerateMethod, mapAttr.FieldGenerateMethodExtraInfo));
        ...
        public static object GetValue(Type type, object value, object default_value, string datetime_format)
        {
