<%@ Page Language="C#" MasterPageFile="~/MP.master" AutoEventWireup="true" CodeFile="Edit.aspx.cs"
    Inherits="Oracle_Edit" Title="Untitled Page" %>
    ...
    <div id="edit-content">
        <div id="dv-table-objects" class="tab">
            ...
            <div style="margin: 0px 0px 0px 5px; width: 779px; min-height: 600px; float: right;">
                <div style="border: solid 1px #336699;margin:0px 0px 5px 0px;" id="dv-table-parameter">
                    <table style="width: 100%;" cellpadding="1px" cellspacing="1px">
                        <tr>
                            <td>
                                Sequence Defined:</td>
                            <td>
                                <input type="text" id="TB_Seq" class="txt" style="width: 200px;" value="" />
                                e.g.:LOGID:SEQ_T_VISITLOG_LOGID;SEQID:SEQ_T_VISITLOG_SEQID
                            </td>
                        </tr>
                        <tr>
                            <td colspan="4" align="right">
                                <input type="button" class="btn5" onclick="GenTableCode2();" value="Generate" /></td>
                        </tr>
                    </table>
                </div>
                    ...
                    <table style="width: 100%;" cellpadding="1px" cellspacing="1px">
                        <tr>
                            <td>
                                Object Name</td>
                            <td>
                                <input type="text" id="QR_Name" required="1" class="txt" style="width: 200px;" value="TableObjectsFields" /></td>
                            <td>
                                Primary Key(s)</td>
                            <td>
                        ...
                        <tr>
                            <td>Remark</td>
                            <td colspan="3"><input type="text" id="QR_Remark" class="txt" maxlength="1000" style="width:80%;" /></td>
                        </tr>
                        <tr>
                            <td valign="top">
                                Query SQL</td>
                            <td colspan="3">
                                <textarea id="QR_Sql" cols="10" rows="10" style="height: 100px; width: 99%;" class="txt">SELECT UA.TABLE_NAME,UA.TABLE_TYPE,UA.COMMENTS FROM USER_TAB_COMMENTS UA ORDER BY UA.TABLE_NAME</textarea>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="4" align="right">
                ...
                <div id="dv-store-procedure-parameters">
                    ...
                    <div style="border: solid 1px #336699; margin-right: 0px; background-color: #ffffff;">
                        <table style="width: 100%;" cellpadding="1px" cellspacing="1px">
                            ...
                            <tr>
                                <td>Remark</td>
                                <td colspan="3"><input type="text" id="SP_Remark" class="txt" maxlength="1000" style="width:80%;" /></td>
                            </tr>
                            <tr>
                                <td valign="top">
                                    Store Procedure</td>
...
{
    CommonCall('Callback/Oracle/Edit.aspx',0,'dv-tables','dv-tables','dv-tables','EP_','&pn='+ProjectName,'text',"GenDoTableCode('USER_TABLES');");
});
...
function GenTableCode2()
{
    InitView('table');
    GenDoTableCode(TableName);
}
function InitView(t)
{
...
{
     //CommonCall('Callback/Oracle/Edit.aspx',1,'dv-table-do-display','dv-table-do','dv-table-do','EP_','&pn='+ProjectName+'&tn='+table+'&hl=1','text',"GenBoTableCode('"+table+"')");
     CommonCall('Callback/Oracle/Edit.aspx',1,'dv-table-do-value','dv-table-do','dv-table-do','EP_','&pn='+ProjectName+'&tn='+table+'&seq='+oa('TB_Seq'),'text',"GenBoTableCode('"+table+"')");
}
function GenBoTableCode(table)
{
     //CommonCall('Callback/Oracle/Edit.aspx',2,'dv-table-bo-display','dv-table-bo','dv-table-bo','EP_','&pn='+ProjectName+'&tn='+table+'&hl=1','text',null);
     CommonCall('Callback/Oracle/Edit.aspx',2,'dv-table-bo-value','dv-table-bo','dv-table-bo','EP_','&pn='+ProjectName+'&tn='+table+'&seq='+oa('TB_Seq'),'text',null);
}
function ftQR(obj)
{
    ft(obj);
...
{
    var parameters='&tn='+oa('QR_Name')+'&sql='+oa('QR_Sql')+'&pk='+oa('QR_PrimaryKeys')+'&remark='+oa('QR_Remark');
    //CommonCall('Callback/Oracle/Edit.aspx',3,'dv-query-do-display','dv-query-do','dv-query-do','QR_','&hl=1&pn='+ProjectName+parameters,'text','GenBoQueryCode();');
    CommonCall('Callback/Oracle/Edit.aspx',3,'dv-query-do-value','dv-query-do','dv-query-do','QR_','&pn='+ProjectName+parameters,'text','GenBoQueryCode();');
}
function GenBoQueryCode()
{
    var parameters='&tn='+oa('QR_Name')+'&sql='+oa('QR_Sql')+'&pk='+oa('QR_PrimaryKeys')+'&remark='+oa('QR_Remark');
    //CommonCall('Callback/Oracle/Edit.aspx',4,'dv-query-bo-display','dv-query-bo','dv-query-bo','QR_','&hl=1&pn='+ProjectName+parameters,'text',null);
    CommonCall('Callback/Oracle/Edit.aspx',4,'dv-query-bo-value','dv-query-bo','dv-query-bo','QR_','&pn='+ProjectName+parameters,'text',null);
}
function ftSP(obj)
{
    ft(obj);
    if(o('dv-store-procedures').innerHTML=='')CommonCall('Callback/Oracle/Edit.aspx',5,'dv-store-procedures','dv-store-procedures','dv-store-procedures','EP_','&pn='+ProjectName,'text',"if(oa('txt-store-procedure-do-value')=='')GenDoStoreProcedure();");
}
function GenStoreProcSimple(spn)
{    
    o('SP_StoreProcedureName').value=spn;
    CommonCall('Callback/Oracle/Edit.aspx',9,'','dv-store-procedure-parameters','','','&pn='+ProjectName+'&spn='+spn,'text',function(data){
        o('SP_ObjectName').value=data;
        CommonCall('Callback/Oracle/Edit.aspx',6,'dv-store-procedure-simple','dv-store-procedure-parameters','dv-store-procedure-simple','EP_','&pn='+ProjectName+'&spn='+spn,'text',null);
    });
}
...
{
    var parameters='&on='+oa('SP_ObjectName')+'&sql='+oa('SP_Sql')+'&spn='+oa('SP_StoreProcedureName')+'&remark='+oa('SP_Remark');
    //CommonCall('Callback/Oracle/Edit.aspx',7,'dv-store-procedure-do-display','dv-store-procedure-do','dv-store-procedure-do','QR_','&hl=1&pn='+ProjectName+parameters,'text','GenBoStoreProcedure();');
    CommonCall('Callback/Oracle/Edit.aspx',7,'dv-store-procedure-do-value','dv-store-procedure-do','dv-store-procedure-do','QR_','&pn='+ProjectName+parameters,'text','GenBoStoreProcedure();');
}
function GenBoStoreProcedure()
{
    var parameters='&on='+oa('SP_ObjectName')+'&sql='+oa('SP_Sql')+'&spn='+oa('SP_StoreProcedureName')+'&remark='+oa('SP_Remark');
   // CommonCall('Callback/Oracle/Edit.aspx',8,'dv-store-procedure-bo-display','dv-store-procedure-bo','dv-store-procedure-bo','QR_','&hl=1&pn='+ProjectName+parameters,'text',null);
    CommonCall('Callback/Oracle/Edit.aspx',8,'dv-store-procedure-bo-value','dv-store-procedure-bo','dv-store-procedure-bo','QR_','&pn='+ProjectName+parameters,'text',null);
}
    </script>
