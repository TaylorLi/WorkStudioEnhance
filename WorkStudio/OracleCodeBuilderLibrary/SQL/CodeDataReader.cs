...
namespace OracleCodeBuilderLibrary.SQL
{
    ...
    {
        public static string GetDataObject(Settings cf, ColumnMapping.ColumnInfos ci, string tableName, string PK, string sql, string queryRemark)
        {
            ...
            string decorateTableName = tableName;
...
//------------------------------------------------------------------------------", Config.Version, DateTime.Now));
            ...
            sb_ret.AppendLine("");
            sb_ret.Append("namespace ").AppendLine(string.Concat(cf.DataObjectNameSpace, ".Query"));
            sb_ret.AppendLine("{");
            if (!string.IsNullOrEmpty(queryRemark))
            {
                sb_ret.AppendLine("\t/// <summary>");
                sb_ret.AppendLine(string.Concat("\t///", "DO Unit:", queryRemark.Replace("\r\n", " ").Replace("\n", " ")));
                sb_ret.AppendLine("\t/// </summary>");
            }
            ...
            foreach (ColumnMapping.ColumnInfo c in ci)
            {
                ...
                string decorateColName = Util.ConvertStyle(c.Column, cf.DecorateStyle);
                ...
                sb_columns.AppendLine(string.Format("\t\t\tprivate {0} _{1};", c.ColumnType, decorateColName));
                ...
                sb_columns.AppendLine(string.Format("\t\t\tpublic {0} {1}", c.ColumnType, decorateColName));
                sb_columns.AppendLine("\t\t\t{");
            ...
            {
                sb_ret.AppendLine("\t\t/// <summary>");
                sb_ret.AppendLine(string.Concat("\t\t///", queryRemark.Replace("\r\n", " ").Replace("\n", " ")));
                sb_ret.AppendLine("\t\t/// </summary>");
            }
        ...
        public static string GetBusinessObject(Settings cf, ColumnMapping.ColumnInfos cis, string tableName, string PK, string queryRemark)
        {
            //string decorateTableName = Util.ConvertStyle(tableName, cf.DecorateStyle);
            string decorateTableName = tableName;
            StringBuilder sb_ret = new StringBuilder();
            ...
            {
                string decoratePKName = Util.ConvertStyle(primaryKeys[i], cf.DecorateStyle);
                string camelPKName = Util.ConvertStyle(primaryKeys[i], CodeDecorateStyle.Camel);
                if (sbParameter.Length > 0) sbParameter.Append(",");
                sbParameter.Append(GetColumnType(cis, primaryKeys[i])).Append(" ").Append(camelPKName);
            ...
            sb_ret.AppendLine("using OracleDataAccess.Data;");
            sb_ret.AppendLine("using DataMapping;");
            sb_ret.Append("using ").Append(string.Concat(cf.DataObjectNameSpace, ".Query")).AppendLine(";");
            sb_ret.AppendLine("");

            sb_ret.Append("namespace ").AppendLine(string.Concat(cf.BusinessObjectNameSpace, ".Query"));
            sb_ret.AppendLine("\t{");
            if (!string.IsNullOrEmpty(queryRemark))
            {
                sb_ret.AppendLine("\t/// <summary>");
                sb_ret.AppendLine(string.Concat("\t///", "BO Unit:", queryRemark.Replace("\r\n", " ").Replace("\n", " ")));
                sb_ret.AppendLine("\t/// </summary>");
            }
            ...
            sb_ret.AppendLine(string.Format("\t\t\treturn da.GetRecordsCount(GetConditionsByObject(parameterObj, isAnd, tokenTypes, extTokens));", cf.DOPrefix, decorateTableName));
            sb_ret.AppendLine("\t\t}");
            ...
            if (primaryKeys != null && primaryKeys.Length > 0)
            {
                ...
                sb_ret.AppendLine(string.Format("\t\t\treturn parameterObj.GetPagingList(GetConditionsByObject(parameterObj, true, TokenTypes.Like,null), pageNumber, pageSize, sortBy, isAsc);", cf.DOPrefix, decorateTableName));
                sb_ret.AppendLine("\t\t}");
            }
            sb_ret.AppendLine("");
            sb_ret.AppendLine("\t\t#endregion");
